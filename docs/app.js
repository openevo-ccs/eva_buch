...
function setActiveNav() {
  ...
  document.querySelectorAll('.nav-links a').forEach((link) => {
    ...
    // Strips a leading "./" or "/" (both appear across this site's markup)
    // and the trailing ".html". Previously only stripped a leading "/", so
    // "./bertopic.html" -> "./bertopic" never matched "bertopic" and this
    // function silently removed the active-nav highlight on every page load
    // -- it happened to look right only because each page's own link also
    // hardcodes class="is-active" directly in the HTML, and nothing had
    // specifically checked whether the JS then stripped it back off.
