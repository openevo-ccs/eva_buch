...
5. ✅ **Done** — Adaptive `nr_topics`-as-ceiling: `DEFAULT_NR_TOPICS_CEILING = 11` in `bertopic_pipeline_v2.py`, now the CLI default. Verified via BERTopic 0.16.4 source (`_reduce_topics`/`_reduce_to_n_topics`) that `nr_topics` only ever merges DOWN when a concept's natural topic count exceeds it — never inflates a concept that naturally settles lower — so no additional custom ceiling logic was needed beyond passing this value. `--nr-topics 0` (or negative) still available for uncapped/auto.
...
3. Re-run BERTopic: recompute embeddings (new model, `--force-embeddings` — note the cache is now model-namespaced so this is close to automatic), refit topics (`--force-topics`), all 22 concepts, base fit at natural granularity + the now-built `topic_sweep.py` across K ∈ {5,7,10,12,15,20} for the coherence/agreement analysis, ship the chosen K per concept to the app. **Not yet executed** — this is the big compute step, recommend a timing pilot on 2-3 concepts first (per Phase 1 item 4, still outstanding) before committing to all 22.
4. Re-run LDA: all 22 concepts × the same K range (cheap relative to BERTopic's embedding step). **Not yet executed.**
5. Regenerate manifest, color maps, `docs/data` mirror via `scripts/build_docs_data.py`. **Not yet executed.**
